...
| Pain point | Solution (CBT)| Feature |
| ----------------------| ------------------------- | ---------------------------------- |
| P1. **충동의 원인/단서(시간·장소·감정)를 모른다**   | S1-1. **Stimulus/Cue 기록 습관화**(SC) <br> S1-2. **단서-충동-결과 연결 교육**(CS 라이트)       | F1-1. **단서·감정·상황 빠른 기록 폼** (US-02, /daily-checkin) <br> F1-2. **Top 단서/시간대 리포트** (GET /dashboard/summary)                       |
| P2. **밤·피곤함 등 취약 시간대에 무너짐**        | S2-1. **시간대 기반 사전계획**(RP) <br> S2-2. **데일리 규칙 점검 루틴**(SC)                     | F2-1. **‘밤 10시 이후 쇼핑 금지’ 규칙 체크** (US-01) <br> F2-2. **오전/저녁 리마인더** (알림/스케줄)                                                     |
| P3. **“세일 놓치면 손해(FOMO)” 같은 자동사고**  | S3-1. **Cognitive Restructuring**(CR) <br> S3-2. **대안사고 카드 반복 노출**            | F3-1. **자동사고 입력 → 대안사고 제안** (US-03) <br> F3-2. **대안사고 카드 보관/재사용률 트래킹** (KPI: 대안사고 사용률)                                          |
| P4. **푸시 알림-앱 진입-결제까지 질주**         | S4-1. **If–Then 계획**(RP; “알림→물 한 잔→앱 종료”) <br> S4-2. **구매 행동 전 지연·차단 루틴**(SC) | F4-1. **If–Then 버튼 체크** (US-04)<br>  F4-2. **지연 수행률 리포트** (KPI: If–Then 실행률)                                                    |
| P5. **구매 전 ‘잠깐 멈춤’ 실패**            | S5-1. **CS(불쾌 결과 상상)로 충동-결과 연결 강화** <br> S5-2. **대체행동 스크립트**(RP)              | F5-1. **CS 미니 가이드 프롬프트**(챗봇 세션 내) <br> F5-2. **대체행동 체크(물 한 잔, 산책 3분 등)** (/daily-checkin)                                       |
| P6. **실천은 하지만 유지/재발 방지가 어려움**      | S6-1. **Relapse Prevention**(유발 조건·대응계획 업데이트) <br> S6-2. **회기별 플랜 버전 관리**     | F6-1. **회기 종료 시 플랜 생성/갱신** (POST /chat/session, plans.active=true) <br> F6-2. **최근 7/14/28일 KPI 대시보드** (무구매일·규칙 준수율 등)          |
| P7. **기록이 번거롭고 금방 지침**             | S7-1. **초미니 폼(10초 내)·선택형 중심** <br> S7-2. **자유서술 선택적 허용 + 요약 반영**              | F7-1. **단계별 폼 최소화**(SC/CR/RP) <br> F7-2. **LLM 요약으로 폼 자동 채움 보조** (POST /chat/session → practice_logs)                           |
| P8. **후회/죄책감 악순환으로 자기효능감 저하**      | S8-1. **진척 가시화·강화**(RP; 성취 피드백) <br> S8-2. **인지적 자기연민 스크립트**(CR 보조)           | F8-1. **무구매일/규칙 준수율 뱃지·그래프** (GET /dashboard/summary) <br> F8-2. **세션 요약에 ‘성공 패턴’ 하이라이트** (POST /chat/session)                  |
| P9. **개인정보/보안 우려**                 | S9-1. **최소 권한·익명 인증 옵션 안내** <br> S9-2. **보안 규칙·암호화 고지**                       | F9-1. **Firebase Auth(이메일/익명)** <br> F9-2. **Firestore 보안 규칙·HTTPS** (Non-Functional)                                           |
| P10. **고비용 심리상담은 부담, 저비용·고접근이 필요** | S10-1. **LLM+RAG 기반 셀프 코칭** <br> S10-2. **주 1회 회기·데일리 1–3분 경량 루틴**            | F10-1. **RAG FAQ(소비·심리 문서 기반)** (POST /rag/answer) <br> F10-2. **주간 회기형 상담 + 데일리 체크** (US-01~05, /daily-checkin, /chat/session) |
...
flowchart LR
    user["사용자"]
    app["모바일 앱(Android)<br/>- 리포트 UI<br/>- 챗봇 인터페이스<br/>- 알림"]
    api["FastAPI 서버<br/>- Chat API<br/>- Summary API<br/>- Spending API"]
    gpt["GPT API + RAG"]
...
erDiagram
    ...
    PROGRESS {
        string planId
        number streakDays
        number ruleAdherenceRate
        number ifThenUsageRate
        number noBuyDays
        timestamp updatedAt
    }
    ...
    USERS ||--o{ PRACTICE_LOGS : has
    PLANS ||--o{ SESSIONS : referencedBy
    SESSIONS ||--o{ PRACTICE_LOGS : generates```
