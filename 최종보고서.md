...
> 표기 규칙  
> - TS = Test Scenario  
> - 기대 결과는 가능한 한 “관찰/검증 가능”하게 작성  
> - (로그/응답필드)는 구현에 따라 “서버 로그” 또는 “API 응답 필드”로 확인
...
10주 동안 주차별 CBT 상담을 진행하고, 세션 종료 시 **주간 요약 + 다음 과제**를 제공한다. 대화는 **주차별 상담 프로토콜(YAML)**과 **CBT 기법 라이브러리(YAML)** 범위 안에서 생성된다.

#### 상세 기능 요구사항

1) 시스템은 사용자의 현재 주차(`current_week`)에 해당하는 **주차별 프로토콜(YAML)**을 로드해야 한다.  
2) LLM은 프로토콜의 **목표/과업/성공기준/허용 기법 범위**를 벗어나지 않도록 상담 발화를 생성해야 한다.  
3) 시스템은 대화를 **세션 단위(thread)**로 저장하고, 각 메시지에 생성 시각/화자(role)/세션 식별자를 포함해야 한다.  
4) 시스템은 사용자의 답변을 기반으로 다음 질문/개입을 구성하되, 필요 시 **CBT 기법 라이브러리(YAML)**를 참조하여 개입 유형을 선택해야 한다.  
5) 주간 상담 종료 조건(예: 목표 달성, 프로토콜 단계 완료 등)을 만족하면, 시스템은 **주간 요약**을 생성해야 한다.  
6) 주간 상담 종료 시, 시스템은 다음 주차까지 수행할 **과제(homework)**를 사용자에게 안내하고 저장해야 한다.  
7) 앱 재접속/화면 이탈 후 복귀 시에도, 해당 Weekly 세션의 대화가 누락 없이 복원되어야 한다.  
8) 프로토콜 로드 실패/필수 필드 누락 시, 시스템은 오류를 기록하고 사용자에게 “상담을 시작할 수 없음” 상태를 안내해야 한다(무한 로딩 금지).

#### 완료 기준(AC)

- AC1: 현재 주차가 N일 때, **N주차 프로토콜**로 상담이 시작된다(주차 mismatch 없음).  
- AC2: 상담 종료 시 **주간 요약 1개 + 과제 안내 1개 이상**이 생성/저장된다.  
- AC3: 앱 재접속 후 Weekly 세션을 열면 **DB에 저장된 메시지가 시간순으로 모두 표시**된다(assistant 응답 누락 없음).
...
주간 상담 외 질문에 대해, **이전 주간 상담 요약/과제 맥락**을 참고하고, **RAG 기반**으로 신뢰도 높은 답변을 제공한다.

#### 상세 기능 요구사항

1) 사용자는 자유 질문을 입력하여 General(FAQ) 상담을 시작/진행할 수 있어야 한다.  
2) 시스템은 질문 처리 시, 사용자의 **이전 주간 상담 요약**과 **직전 과제 맥락**을 참조할 수 있어야 한다(가능한 범위 내에서).  
...
사용자가 과제 수행과 상담 지속을 잊지 않도록, 알림으로 행동을 유도한다.

#### 상세 기능 요구사항

1) 시스템은 주간 상담 종료 시, 사용자의 다음 행동(과제 수행/다음 상담 등)에 대한 **리마인더 알림을 예약**할 수 있어야 한다.  
2) 알림은 최소 1종 이상 지원해야 한다: (예) “과제 리마인더”, “주간 상담 리마인더”.  
3) 알림은 사용자가 앱을 설치한 상태에서 지정 시각에 표시되어야 한다(플랫폼 정책 준수).  
4) 사용자가 프로그램을 초기화(F6)하면, 기존 진행 기준으로 예약된 알림은 **새 진행 상태와 충돌하지 않도록 갱신/해제**되어야 한다.  
5) 알림이 트리거되면 사용자를 앱의 관련 화면(예: 과제/채팅)으로 유도할 수 있어야 한다(딥링크 또는 앱 진입).

#### 완료 기준(AC)

- AC1: 주간 상담 종료 후, 지정된 시각에 **알림이 실제로 표시**된다.  
- AC2: 초기화 수행 후에는, 이전 주차 기준 알림이 **중복으로 계속 울리지 않는다**.  

#### 테스트 시나리오

- **TS-F3-01: 주간 상담 종료 후 알림 예약(정상)**
  - 사전조건: 알림 권한 허용
  - 절차: WEEKLY 세션 종료 → (테스트용으로 1~3분 후 알림 예약되도록 설정) → 대기
  - 기대결과: 지정 시각에 알림이 표시됨(AC1)  
- **TS-F3-02: 알림 탭 시 앱 유도(정상)**
  - 사전조건: TS-F3-01로 알림 생성 상태
  - 절차: 알림 탭
  - 기대결과: 앱이 열리며 관련 화면(채팅/과제)로 이동(딥링크/내비게이션 확인)  
- **TS-F3-03: 초기화 후 알림 중복 방지(예외/회귀)**
  - 사전조건: 기존 주차 기준 알림이 예약되어 있음
  - 절차: 초기화(F6) 실행 → 일정 시간 동안 알림 관찰
  - 기대결과: 이전 주차 기준 알림이 중복으로 발생하지 않음 / 새 진행 상태 기준으로만 동작(AC2)  
...
### F5. 새로운 FAQ 상담 시작(New General Thread)

#### 목적
기존 FAQ 맥락과 분리된 새 질문 세션을 열어, 주제 전환/정리된 상담을 가능하게 한다.

#### 상세 기능 요구사항
1) 사용자는 “새 FAQ 상담 시작” 동작을 통해 **새 General 세션(thread)**을 생성할 수 있어야 한다.  
2) 새 세션은 기존 General 세션의 맥락을 **승계하지 않고** 0부터 시작해야 한다.  
3) 생성된 세션은 히스토리에 표시되고, 이후 질문은 해당 세션에 저장되어야 한다.

#### 완료 기준(AC)
- AC1: “새 FAQ 시작” 후 첫 질문은 **새 sessionId**에 저장된다.  
- AC2: 기존 FAQ 세션의 메시지가 새 세션에 섞이지 않는다.

#### 테스트 시나리오
- **TS-F5-01: 새 FAQ 세션 생성(정상)**
  - 사전조건: 기존 GENERAL 세션 1개 이상 존재
  - 절차: “새 FAQ 상담 시작” 실행 → 첫 질문 입력
  - 기대결과: 새 sessionId가 생성되고 첫 메시지가 새 sessionId로 저장됨(AC1)  
- **TS-F5-02: 맥락 분리 확인(정상)**
  - 사전조건: 기존 GENERAL 세션에 특정 키워드 포함 대화가 존재
  - 절차: 새 FAQ 세션에서 “방금 전에 얘기한 그 내용 뭐였지?” 같은 질문 입력
  - 기대결과: 기존 세션 맥락을 당연히 아는 듯한 답변이 아니라, 새 세션 기준으로 응답(또는 추가 질문으로 맥락 요청) / 메시지 혼입 없음(AC2)  
...
> <그림 1> 전체 시스템 구성도  
<img width="3840" height="624" alt="Image" src="https://github.com/user-attachments/assets/68d690bc-2409-4b22-9ec4-60410eef6c4e"/>
...
- **Serverless Host (Google Cloud Run)**
