...
- **팀 정보**: 28팀 e스타트
- **팀 구성원**
    - 유희정(1976250): 팀 리더, 기획, 프론트엔드, 챗봇, 보고서 작성
    - 최윤우(2176387): 백엔드, 기획, API 설계, 챗봇, 보고서 작성
    - 이경림(1871033): 기획, RAG
...
<img width="500" alt="Image" src="[https://github.com/user-attachments/assets/3a121191-ce81-48cc-a17e-8dd7c9ef00f7](https://github.com/2025-eStart/TETS/issues/4#issue-3608110865)" />
> 
...
사용자가 API(`/chat/start` 또는 `/chat/send`)를 호출하면, 그래프는 가장 먼저 `LoadState` 노드를 실행해 사용자의 현재 상태(`current_week` 등)를 DB에서 불러옵니다. 그 직후 `RouteSession` 노드가 세션의 "종류"를 결정합니다.

**현재 코드(`app/graph/route_session.py`)의 로직은 다음과 같습니다:**

1. **21일 비활성 체크:** 사용자가 21일 이상 접속하지 않았다면, `current_week`를 1로 리셋하고 **"WEEKLY"** (상담) 세션으로 강제 라우팅합니다.
2. **활성 세션 체크:** `LoadState`가 DB에서 `state.weekly_session`을 성공적으로 불러왔다면 (즉, 아직 진행 중인 주간 상담이 있다면) **"WEEKLY"** (상담) 세션으로 라우팅합니다.
3. **그 외:** 위 두 조건에 해당하지 않으면(예: 프로그램 완료자, 첫 방문은 아님) **"GENERAL"** (일반) 세션으로 라우팅합니다.

**1. 상담 세션 (Weekly)**

- 프로토콜 로드 (`PickWeek`)
    - `LoadState`에서 확인된 `state.current_week`(예: 3주차)에 해당하는 `protocols/v1/week3.yaml` 파일을 불러옵니다.
    - YAML 파일의 `goals`, `script_steps`, `exit_criteria` 등을 `state.protocol`에 저장합니다.
    - 만약 `state.weekly_session`이 없다면(이번 주차 첫 방문), `REPO.create_weekly_session`을 호출해 DB에 새 세션을 생성합니다.
- 기억(메모리) 로드 (`BuildPrompt`)
    - `_load_history` 함수가 **두 종류의 기억**을 불러옵니다.
        - **과거 요약본:** `REPO.get_past_summaries`를 호출해, (현재 3주차라면) 1, 2주차의 **'세션 요약본'**들을 모두 가져옵니다.
        - **현재 대화:** `REPO.get_messages`를 호출해, **'현재 3주차'**에 사용자와 나눈 모든 상세 대화(WEEKLY, DAILY 모두)를 가져옵니다.
    - `SYSTEM_TEMPLATE`에 `state.protocol`의 목표(`goals`), 단계(`script_steps`), 종료 조건(`exit_goals`)을 채워 넣습니다.
    - [과거 요약본 + 현재 대화]를 `history`로, 사용자 입력을 `user_message`로 하여 최종 프롬프트를 완성합니다.
- LLM 응답 및 종료 판단 (`RunLLM`)
    - `LLM_CHAIN`이 구조화된 출력(`.with_structured_output(CounselorTurn)`)을 호출합니다.
    - LLM은 **두 가지**를 반환합니다.
        1. `response_text` (사용자에게 보낼 답변) → `state.llm_output`에 저장
        2. `session_goals_met` (YAML의 종료 조건을 만족했는지) → `state.exit`에 저장
- 턴 저장 및 '요약' 생성 (`SummarizeUpdate`)
    - `persist_turn`을 호출해 방금 나눈 대화(사용자 입력, AI 응답)를 DB에 **'턴' 단위로 저장**합니다.
    - **[핵심 분기]** `if state.exit == True:` (즉, LLM이 방금 3주차 상담이 끝났다고 판단했다면)
        - `_create_summary` 함수가 LLM을 **한 번 더 호출**합니다.
        - 3주차의 '모든' 상세 대화 내용을 기반으로 **'3주차 요약본'**을 생성합니다.
        - `REPO.save_session_summary`를 호출해 이 요약본을 DB에 저장합니다. (다음 4주차 세션이 이 요약본을 사용하게 됩니다.)
    - `state.exit`가 `False`라면, '턴' 저장만 하고 요약 생성은 건너뜁니다.
- 종료 (`END`)
    - 그래프가 종료되고, `main.py`는 `state.llm_output`에 담긴 AI 응답을 사용자에게 반환합니다.
1. **일일세션 (Daily)**
- 프로토콜 로드 (건너뜀)
    - `cond_route_session`이 "DAILY"를 반환하면 `PickWeek` 노드를 건너뛰고 바로 `BuildPrompt`로 갑니다.
    - `state.protocol`은 비어있게 됩니다.
- 기억(메모리) 로드 (`BuildPrompt`)
    - `_load_history` 함수가 `session_type == "DAILY"` 조건을 확인합니다.
    - **과거 요약본:** 1, 2주차의 **'세션 요약본'**을 모두 가져옵니다. (상담 내용 참고)
    - **현재 대화:** `_load_history` 로직에 따라, 현재 3주차의 대화 중 `session_type == "weekly"`인 상세 대화만 가져옵니다. (즉, **과거의 일일 대화는 불러오지 않습니다.**)
    - `state.protocol`이 비어있으므로, `SYSTEM_TEMPLATE`은 기본값("Daily Check-in")으로 채워집니다.
- LLM 응답 및 종료 판단 (`RunLLM`)
    - `CounselorTurn`을 호출합니다.
    - `state.protocol`의 `exit_criteria`가 없으므로, LLM은 `session_goals_met=False`를 반환할 가능성이 높습니다. (즉, `state.exit`는 항상 `False`입니다.)
- 턴 저장 (`SummarizeUpdate`)
    - `persist_turn`을 호출해 '일일 대화' 턴을 DB에 저장합니다.
    - `if state.exit == False` 이고 `state.session_type == "DAILY"` 이므로, **'세션 요약'은 절대 생성되지 않습니다.**
1. 일반 세션 (General)
- 프로토콜 로드 (건너뜀)
    - "GENERAL"로 라우팅되면 `PickWeek`을 건너뛰고 `BuildPrompt`로 갑니다.
- 기억(메모리) 로드 (`BuildPrompt`)
    - `_load_history` 함수가 `session_type == "GENERAL"` 조건을 확인합니다.
    - 현재 코드(`else: pass`)에 따라, **아무런 기억(요약본, 상세 대화)도 불러오지 않습니다.**
    - `state.protocol`도 비어있으므로, 완전한 기본 프롬프트로 LLM을 호출합니다.
- LLM 응답 및 종료 판단 (`RunLLM`)
    - `state.exit`는 항상 `False`가 됩니다.
- 턴 저장 (`SummarizeUpdate`)
    - `persist_turn`을 호출해 '일반 대화' 턴을 DB에 저장합니다.
    - **'세션 요약'은 생성되지 않습니다.**
